export class OTP {
    constructor(options = {}) {
        ...
        options.codeLength = Number.isNaN(options.codeLength ?? Number.NaN) ? 6 : options.codeLength;
        options.secret = options.secret || generateKey(options.keySize);
        options.epoch = Number.isNaN(options.epoch ?? Number.NaN) ? 0 : options.epoch;
        options.timeSlice = Number.isNaN(options.timeSlice ?? Number.NaN) ? 30 : options.timeSlice;
...
export default OTP;
async function hotp(options, counter) {
...
async function totp(options, now = Date.now()) {
...
export function decodeChunk(data, dest) {
    ...
    if (isNumber(c3) && isNumber(c4))
        dest[1] = byte((c2 << 6) | (c3 << 1) | (c4 >> 4)); // xxx12 34567 8xxxx
    if (isNumber(c4) && isNumber(c5))
        dest[2] = byte((c4 << 4) | (c5 >> 1)); // x1234 1234x
    if (isNumber(c5) && isNumber(c6) && isNumber(c7))
...
function assertNumber(arg) {
    if ('number' !== typeof arg || !isNumber(arg))
